...
-- =============================================================================
-- EXAMPLE DATA TYPES FOR APPLICATION
-- =============================================================================

-- Enum-like constraints (can be implemented as CHECK constraints or separate tables)
-- Channel types: 'api', 'email'
-- Processing status: 'processed', 'failed', 'manual_review'  
-- Message status: 'pending', 'replied', 'failed', 'expired'
-- Delivery status: 'sent', 'delivered', 'bounced', 'spam', 'opened'
-- Send timing: 'immediate', 'office_hours', 'scheduled'
-- Campaign status: 'active', 'archived', 'closed'
-- Politician level: 'national', 'regional', 'local', 'european'
...
/*
-- Find similar messages for campaign classification
SELECT id, campaign_id, message_embedding <-> $1::vector as distance 
FROM messages 
ORDER BY message_embedding <-> $1::vector 
LIMIT 10;

-- Campaign stats for politician dashboard  
SELECT 
    c.name,
    COUNT(*) as message_count,
    COUNT(CASE WHEN m.received_at > NOW() - INTERVAL '7 days' THEN 1 END) as recent_count,
...
AND m.politician_id = $1
GROUP BY c.id, c.name
ORDER BY message_count DESC;

-- Messages pending reply for a politician (fetch personal data via JMAP)
SELECT 
    m.id,
    m.external_id, -- use this to fetch full message via JMAP
...
-- Find politician by any of their emails
SELECT find_politician_by_email('politician@example.com');
*/
