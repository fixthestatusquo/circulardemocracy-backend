...
describe("URL Utilities", () => {
  it("should extract subdomain from URL", () => {
    ...
    expect(extractSubdomain("https://climate.example.com/page")).toBe(
      "climate",
    );
    expect(extractSubdomain("https://action.subdomain.example.com/page")).toBe(
      "action",
    );
  ...
  it("should extract text from HTML", () => {
    ...
    const html =
      "<html><head><title>Test</title></head><body><h1>Climate Action</h1><p>We need urgent action on climate change.</p></body></html>";
  ...
  it("should remove script and style tags", () => {
    ...
    const html =
      '<p>Visible text</p><script>alert("hidden")</script><style>.hidden { display: none; }</style>';
    const text = extractTextFromHtml(html);
