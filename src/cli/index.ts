...
USAGE:
  ...
  cli --fetch-from-stalwart --process-all
  cli --fetch-from-stalwart --since "2024-03-01"
  cli --fetch-from-stalwart --message-id "specific-id"
  cli --fetch-from-stalwart --dry-run [--process-all|--since <date>|--message-id <id>]
...
OPTIONAL ARGUMENTS:
  ...
  Stalwart fetch mode:
  --fetch-from-stalwart  Enable Stalwart/JMAP ingestion mode
  --process-all          Fetch all available messages (default when no filter provided)
  --since                Fetch messages received after date
  --message-id           Fetch one specific message (JMAP ID or Message-ID header)
  --dry-run              Preview converted messages without processing/storage

STALWART ENV VARIABLES:
...
async function generateMockEmbedding(text: string): Promise<number[]> {
  ...
  const embedding = new Array(1024).fill(0); // Match DB vector dimension
  let hash = 0;
...
async function runStalwartIngestion(
  ...
  const username = process.env.STALWART_USERNAME || "dibora";
  const password = process.env.STALWART_APP_PASSWORD || process.env.STALWART_PASSWORD;

  if (!password) {
    console.error("Error: STALWART_APP_PASSWORD environment variable must be set");
...
async function main() {
  ...
  const isStalwartFetchMode = args.includes("--fetch-from-stalwart");
